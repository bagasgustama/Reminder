...
class LoginPage extends RunnerPage
{
	...
	protected function getSelectSQL( $skipPasswordCheck, $strUsername, $strPassword, $loginSet, $cipherer )
	{
		...
		return "select * from ".$this->connection->addTableWrappers("personal")
	...
	public function LoginWithSP( $plugin, $token, $addNewUser = true )
	{
		...
		$sqlHead = "select * from ".$this->connection->addTableWrappers( "personal" ) ." where ";
		$sql = array();
		if( $userIdField ) {
			$sql []= $sqlHead
		...
		else if( $addNewUser )
		{
			...
			$blobfields = array();
			$retval = DoInsertRecord( "personal", $data, $blobfields, $this );
			if( !$retval )
				return false;
